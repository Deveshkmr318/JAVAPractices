...
public class E_Secret_Box{  
    ...
    private static int countPlacements(int x, int y, int z, long k) {
        List<int[]> factors = new ArrayList<>();

        // Find all factors of k and store them as possible dimensions
        for (int i = 1; (long)i * i * i <= k; i++) {
            if (k % i == 0) {
                for (int j = i; (long)j * j <= k / i; j++) {
                    if ((k / i) % j == 0) {
                        int a = i;
                        int b = j;
                        int c = (int)(k / (i * j));
                        addUniqueCombinations(factors, a, b, c);
                    }
                }
            }
        }
        ...
        // Check which combinations fit in the box B
        for (int[] dim : factors) {
            int a = dim[0], b = dim[1], c = dim[2];
            if (a <= x && b <= y && c <= z) {
                int placements = (x - a + 1) * (y - b + 1) * (z - c + 1);
                maxPlacements = Math.max(maxPlacements, placements);
            }
        }
    ...
    private static void addUniqueCombinations(List<int[]> factors, int a, int b, int c) {
        factors.add(new int[]{a, b, c});
        factors.add(new int[]{a, c, b});
        factors.add(new int[]{b, a, c});
        factors.add(new int[]{b, c, a});
        factors.add(new int[]{c, a, b});
        factors.add(new int[]{c, b, a});
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < N; i++) {
            ...
            out.println(countPlacements(x,y,z,k));
