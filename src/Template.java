...
{   
    ...
    public static void main(String[] args)
    {
        ...
        for(int i = 0 ; i < t ; i++){
            ...
            // System.out.println((flag || k > 1)?"YES":"NO");
    ...
    public static int intsBetweenStream(final double num1, final double num2) {
        ...
        return (int)IntStream.range(below, above).count() + 1;
    }
    ...
    static class Question {
        ...
        public void solve(int testNumber, InputReader sc, OutputWriter out) {
            ...
            int map[] = new int[12];
            long pre[] = new long[12];
            pre[0] = 1;
            for (int i = 1; i < 11; i++) {
                pre[i] = modularMultiplication(pre[i - 1], 10, mod);
            }
            long sum = 0;
            for (int i = n - 1; i >= 0; i--) {
                sum = modularAddition(sum, modularMultiplication(arr[i], i, mod), mod);
                for (int j = 0; j < 11; j++) {
                    // sum += arr[i] * map[j] * pre[j];
                    sum = modularAddition(sum,
                            modularMultiplication(arr[i], modularMultiplication(map[j], pre[j], mod), mod), mod);
                }
                long num = arr[i];
                int count = 0;
                while (num > 0) {
